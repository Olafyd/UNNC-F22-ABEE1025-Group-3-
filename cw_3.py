...
for i in parameter_vals:
	epjson_new = epjson
	epjson_new['Material:NoMass']['R31LAYER']['solar_absorptance'] = i
	with open(f'epjson_path_sa{i}.epJSON', 'w') as f:
...
eplus_output_dir = './result'
for i in parameter_vals:
    input_file = f'epjson_path_sa{i}.epJSON'
    output_file = eplus_output_dir + f'/run_result{i}'
    cl_st = (f'"{eplus_run_path}"'
            + f'--output-prefix solar_absorptance{i}'
            + '--readvars ' 
